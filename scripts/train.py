...
def get_augmentation_layer():
    ...
    return keras.Sequential([
        ...
        layers.RandomTranslation(0.1, 0.1),  # NEW: small shifts
    ], name="augmentation")
...
def create_dataset(directory, is_training=True, batch_size=32):
    ...
    dataset, class_names = keras.preprocessing.image_dataset_from_directory(
        directory,
        image_size=(224, 224),
...
def get_callbacks(config, phase_name):
    ...
    return [
        ...
        keras.callbacks.EarlyStopping(
            ...
            verbose=1,
            min_delta=1  # Only stop if improvement < 1
        ),
...
def train():
    ...
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=config.LEARNING_RATE_PHASE1), # pyright: ignore[reportArgumentType]
    ...
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=config.LEARNING_RATE_PHASE2), # pyright: ignore[reportArgumentType]
        loss=keras.losses.SparseCategoricalCrossentropy(from_logits=True),
        metrics=[
            keras.metrics.Accuracy(),
            keras.metrics.SparseTopKCategoricalAccuracy(k=2, name='top2_acc')
    ...
    test_loss, test_acc, test_top2 = model.evaluate(test_ds, verbose=0) # pyright: ignore[reportArgumentType]
